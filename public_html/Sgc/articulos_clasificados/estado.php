...
?>
<?php
if ((isset($_GET['id_art'])) && ($_GET['id_art'] != "")) {
  $esta = $_GET['estado'];
 
  $updateSQL = "UPDATE articulos_clasificados SET Estado_Articulo = '$esta' WHERE Id_Articulo = ".$_GET['id_art'];
...
<? mysql_select_db($database_cnx_arica, $cnx_arica);
$query_rsNoticia= "SELECT * FROM articulos_clasificados WHERE Id_Articulo = ".$_GET['id_art'];
...
<option value="1" <?php if (!(strcmp(1, $row_rsNoticia['Estado_Articulo']))) {echo "selected=\"selected\"";} ?>>Habilitado</option>
<option value="0" <?php if (!(strcmp(0, $row_rsNoticia['Estado_Articulo']))) {echo "selected=\"selected\"";} ?>>Deshabilitado</option>
