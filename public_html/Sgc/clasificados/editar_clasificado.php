...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  $insertSQL = sprintf("INSERT INTO categoria (Nombre_Cat) VALUES (%s,%s,$s)",
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE clasificados SET  titulo_Clasificado=%s,Descripcion_Cla=%s,fecha_clasificado=%s, Estado_Cla=%s WHERE Id_Clasificados=%s",	
   					   GetSQLValueString($_POST['titulo_Txt'], "text"),
                       GetSQLValueString($_POST['nombre_Txt'], "text"),
					    GetSQLValueString($_POST['fecha_Txt'], "text"),
  ...
  $updateGoTo = "categoria.php?id_cat=".$_GET['id_cat'];
  if (isset($_SERVER['QUERY_STRING'])) {
...
if (isset($_GET['id_pro'])) {
  $colname_rsCategoria = $_GET['id_pro'];
}
mysql_select_db($database_cnx_arica, $cnx_arica);
$query_rsCategoria = sprintf("SELECT * FROM clasificados WHERE Id_Clasificados= %s", GetSQLValueString($colname_rsCategoria, "int"));
...
<table width="1003" border="0" align="center" cellpadding="0" cellspacing="0" class="bordes">
  ...
  <tr>
    <td><blockquote>
      <p class="titles">&raquo; M&oacute;dulo de Clasificados - Editar Clasificado</p>
      ...
      <input name="titulo_Txt" type="text" class="fuente" id="titulo_Txt" title="Nombre de Categor�a" value="<?php echo $row_rsCategoria['titulo_clasificado'];?>"/>
      </td>
                 </tr>
                  ...
                  <tr>
                    <td align="right" class="fuente"><strong>Descripci&oacute;n Clasificado:</strong></td>
                    <td><label for="categoria_Txt2"></label>
                      <label for="nombre_Txt"></label>
                        <textarea name="nombre_Txt" type="text" class="fuente" id="nombre_Txt" title="Nombre de Categor�a" cols="45" rows="5"><?php echo $row_rsCategoria['Descripcion_Cla']; ?></textarea>
           <input name="fecha_Txt" type="hidden" class="fuente" id="fecha_Txt" size="35" title="Fecha" value="<? $fecha=date('Y-m-d H:i:s');echo $fecha;?>">             
                  ...
                  <tr>
                    ...
                    <td width="54%" class="fuente"><label for="categoria_Txt"></label>
                      <label for="nombre_Txt"></label>
                      <input <?php if (!(strcmp($row_rsCategoria['Estado_Cla'],1))) {echo "checked=\"checked\"";} ?> name="estado_Txt" type="checkbox" id="estado_Txt" value="1">
                      <label for="estado_Txt">Habilitado</label></td>
                  </tr>
                  <tr>
                    <td colspan="2" align="center">
                      <input type="hidden" name="MM_update" value="form1">
                      <input name="id_cat" type="hidden" id="id_cat" value="<?php echo $row_rsCategoria['Id_Clasificados']; ?>">
          ...
          <p><a href="index.php" class="fuente linksRojo"><strong>&laquo; Volver al Listado de Categorias</strong></a></p>
