...
$query_rsProductos = "SELECT * FROM clasificados, categoria_clasificado WHERE  clasificados.IdCategoriaCla_Cla = '$id_cat' AND noticias.Titulo_Not like '%$buscar%' AND   categoria_clasificado.Id_CategoriaClasificado= '$id_cat' ORDER BY  categoria_clasificado.Nombre_CatCla ASC ";
...
$query_rsProductos = "SELECT * FROM clasificados, categoria_clasificado WHERE clasificados.IdCategoriaCla_Cla ='$id_cat' AND categoria_clasificado.Id_CategoriaClasificado= '$id_cat'     ";
...
<table width="1003" border="0" align="center" cellpadding="0" cellspacing="0" class="bordes">
  ...
  <tr>
    <td><blockquote>
      <p class="titles">&raquo; M&oacute;dulo de Clasificados - Listado  de  Categor&iacute;a: <?php echo $row_rsCategoria['Nombre_CatCla']; ?></p>
    <?  $id_sub= $_GET['id_sub'];?>
   <?   $id_cat= $_GET['id_cat']?>
     <? if ($totalRows_rsProductos>0) { ?>
         <form action="categoria.php?id_cat=<? echo  $id_cat ?>" method="POST" id="form1">
                  <table width="570" cellpadding="3">
                    <tbody>
                      <tr>
                        <td width="35%" class="fuente" height="28" align="right">Buscar en esta Categor�a</td>
       ...
       <? if ($totalRows_rsProductos>0) { ?>
        <table width="100%" align="center" class="bordes2" >
          <tr align="center" bgcolor="#660000" class="blanco11">
            <td width="34%" bgcolor="#003366" class="blanco12">T&iacute;tulo del Clasificado</td>
            <td width="34%" bgcolor="#003366" class="blanco12">Descripci&oacute;n del Clasificado</td>
             <td width="34%" bgcolor="#003366" class="blanco12">fecha del Clasificado</td>
          ...
          <tbody class="tabla">
            ...
            <tr class="odd">
             ...
             <td class="fuente">-&nbsp;
            <strong><?  echo $row_rsProductos['titulo_clasificado']; ?></strong>
            </td>
            
            
            
              <td class="fuente">-&nbsp;
            <strong><?  echo $row_rsProductos['Descripcion_Cla']; ?></strong></td>
            
            
              <td class="fuente">-&nbsp;
            <strong><?  echo $row_rsProductos['fecha_clasificado']; ?></strong></td>
            
            
            
             <td class="fuente"> 
                        <a href="categoria.php?id_cat=<?php echo $row_rsProductos['Id_Categoria']; ?>" class="fuente"><?php echo $row_rsProductos['Nombre_CatCla']; ?>
              ...
              <td align="center"><select name="estado<?php echo $row_rsProductos['Id_Clasificados'];?>" class="fuente" id="estado<?php echo $row_rsProductos['Id_Clasificados']; ?>" onChange="cargar('estado_clasificado.php?id_cla=<?php echo $row_rsProductos['Id_Clasificados']; ?>&estado='+this.value,'estado<?php echo $row_rsProductos['Id_Clasificados']; ?>&id_cat=<? echo $_GET['id_cat']?>')">
                  <option value="1" <?php if (!(strcmp(1, $row_rsProductos['Estado_Cla']))) {echo "selected=\"selected\"";} ?>>Habilitado</option>
                  <option value="0" <?php if (!(strcmp(0, $row_rsProductos['Estado_Cla']))) {echo "selected=\"selected\"";} ?>>Deshabilitado</option>
                </select></td>
              
                  
              
              
              <td align="center"><a href="editar_clasificado.php?id_pro=<?php echo $row_rsProductos['Id_Clasificados']; ?>&id_cat=<?= $_GET['id_cat']?>"><img src="../img/icon_edit.png" width="20" height="20" border="0"></a></td>
                <td align="center"><a href="javascript:preguntarclasificado('<?php echo $row_rsProductos['Id_Clasificados']; ?>','<?php echo $row_rsProductos['Titulo_Not']; ?>')"><img src="../img/icon_del.png" width="19" height="20" border="0"></a></td>
              </tr>
...
Clasificados del <?php echo ($startRow_rsProductos + 1) ?> al <?php echo min($startRow_rsProductos + $maxRows_rsProductos, $totalRows_rsProductos) ?> de <?php echo $totalRows_rsProductos ?> en total</td>
            ...
            <p><a href="agregar_clasificado.php?id_cat=<? echo $id_cat?>" class="fuente linksRojo"><strong>[+] Agregar Nuevo Clasificado</strong></a></p>
