...
if ((isset($_GET['id_usu'])) && ($_GET['id_usu'] != "")) {
  $esta = $_GET['estado'];
 	$updateSQL = "UPDATE usuarios SET Estado_Usu= '$esta' WHERE Id_Usuario = ".$_GET['id_usu'];
...
<? mysql_select_db($database_cnx_arica, $cnx_arica);
$query_rsNoticia= "SELECT * FROM usuarios WHERE Id_Usuario= ".$_GET['id_usu'];
...
<option value="1" <?php if (!(strcmp(1, $row_rsNoticia['Estado_Usu']))) {echo "selected=\"selected\"";} ?>>Habilitado</option>
<option value="0" <?php if (!(strcmp(0, $row_rsNoticia['Estado_Usu']))) {echo "selected=\"selected\"";} ?>>Deshabilitado</option>
