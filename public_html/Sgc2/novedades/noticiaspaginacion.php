...
<? include('../../0_SOURCE/SGC_INTERNET/Connections/cnx_arica.php');?>
...
?>
	...
	<div style="height:170px;width:260px;display:inline;float:left;margin-right:10px"><img width="260" height="140" align="left" style="margin-right:10px" src="../novedades/img/<?php echo $row_rsImagen2['Archivo_Gal']; ?>" alt="" />&nbsp;<? 
$palabra=$row_rsImagen2['Descripcion_Gal'];
 ...
 ?>
				...
				<a id="noticias-titulo"  href="../../novedades/detalle.php?id_not=<? echo $row_rsImagen2['Id_Noticia']?>"
                <? /*"javascript:cargar('../novedades/detalle_noticia.php?id_not=<? echo $row_rsImagen2['Id_Noticia']?>','cuerpo');seleccionado(li<? echo $row_rsImagen2['Id_Categoria'] ?>)"><h1 class="titulodetalenoticiadivnoticia" style="font-size:20px" */?> > 
				<? echo utf8_encode($row_rsImagen2['Titulo_Not'])?>
                
                </h1></a>
                 <h2 style="color:#FF6600; font-size:13px; font-weight:bold"> 
				
                
				 <a href="javascript:cargar('../novedades/noticiaspaginacion.php?id_tip=<? echo $id_cat?>','cuerpo')
                 ;seleccionado(li<? echo $row_rsImagen2['Id_Categoria']?>)" >
				 <? echo utf8_encode($row_rsImagen2['Nombre_Cat'])?>  </a></h2>
...
<br />
			<div style="float:right;text-align:justify;width:420px; margin-right:10px;font-size:11px;font-family:Arial">
				...
				$palabra=$row_rsImagen2['Desarrollo_Not'];
						...
						echo utf8_encode($palabra_cortada)."  ...";}
					 ?><a style="color:#814E62; font-weight:bold;" href="../../novedades/detalle.php?id_not=<? echo $id_not?>"<? /*"javascript:cargar('../novedades/detalle_noticia.php?id_not=<? echo $id_not?>','featured') */ ?>" > Leer M&aacute;s</a>
		  </p>
          </div>
 ...
 <? if ($totalPages_rsNoticias>0){?>
    <div style="width:700px;height:50px;float:left">
       ...
       <? if (($numerodepagina-1) >=0){?>
        <a href="javascript:cargar('../novedades/noticiaspaginacion.php?<? if ($id_tip){?>id_tip=<?=$id_tip?><? } ?>&numerodepagina=<? echo ($numerodepagina - 1);?>','featured')">
      Anterior</a> <? }?>
                <? for ($i=0; $i<=$totalPages_rsNoticias;$i++){?>
                
    <a href="javascript:cargar('../novedades/noticiaspaginacion.php?<? if ($id_tip){?>id_tip=<?=$id_tip?><? } ?>&numerodepagina=<? echo $i;?>','featured')" >
			   <? echo $i+1?>
                </a>
              
                <? }?>
               
          <? if (($numerodepagina+1)<=$totalPages_rsNoticias){?>   <a href="javascript:cargar('../novedades/noticiaspaginacion.php?<? if ($id_tip){?>id_tip=<?=$id_tip?><? } ?>&numerodepagina=<? echo ($numerodepagina + 1);?>','featured')">Siguiente</a><? }?>
